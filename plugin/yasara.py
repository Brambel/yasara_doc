...
the plugin repository at <www.yasara.org/plugins>.
...
All plugins must be stored in YASARA's 'yasara/plg' subdirectory (in MacOSX, this folder
...
<P>yasara.selection[i].radiobutton<P>  | The number of the selected radiobutton in the ith selection window (1=first, 2=second.., None if there was no radiobutton).
...
after each command. If the plugin issues hundreds of commands,
...
(See the <Console> command for more details and note that YASARA will not redraw the screen
unless you tell it to, and may thus appear frozen. So don't forget to enable the console
...
<RunPlugin> command in connection with <SavePLI>:
...
Without any selections, no graphical user interface is needed, and plugins can
also be run in console mode:
<P>
...
*) Write a {Python module<Scripts>} that opens a socket and listens for
...
from the Windows Task Manager, with the Linux 'kill' command or with the MacOSX
...
class sel_descriptor:
  ...
  # SET RADIO BUTTON
  # ================
  def setradiobutton(self,string):
...
def runretval(command,retvalused):
  ...
  if (not retvalused):
    ...
    if (com==None):
      ...
      # SEND COMMAND ONLY
      sys.stdout.write('ExecRV: '+command+'\n')
      sys.stdout.flush()
...
def selstr(selection):
  if (type(selection) in [type(""),type(1),type(1.)]): selstr=cstr(selection)
  else:
  ...
  if (os.path.exists(inputfilename)):
    ...
    for line in input:
      ...
      # ADD TO CURRENT SELECTION
        ...
        selection[0].setradiobutton(line[15:])
...
if (request!="CheckIfDisabled"):
  storage=runretval("LoadStorage",1)
...
# SET/GET ACCELERATION OF ATOMS (ALL IMPLICITLY)
# ==============================================
...
# ADD N/C-TERMINAL CAPPING GROUPS (ALL IMPLICITLY)
# ================================================
...
# ADD TO ENVIRONMENT FOR SURFACE CALCULATIONS (ALL IMPLICITLY)
# ============================================================
def AddEnv():
  command='AddEnv '
  return(runretval(command[:-1],retvalused()))
...
# ADD MISSING HYDROGENS (ALL IMPLICITLY)
# ======================================
...
def AddRes2(name, selection1, epsilon=None, zeta=None, alpha=None, beta=None, gamma=None):
  command='AddRes '
  command+='Name='+cstr(name)+','
  command+=selstr(selection1)+','
...
# ADD C-TERMINAL OXYGENS (ALL IMPLICITLY)
# =======================================
def AddTer():
  command='AddTer '
  return(runretval(command[:-1],retvalused()))
...
# ADD OBJECTS TO THE SOUP (ALL IMPLICITLY)
# ========================================
def Add():
  command='Add '
  return(runretval(command[:-1],retvalused()))
...
# ALIGN MULTIPLE OBJECTS (ALL IMPLICITLY)
# =======================================
...
# MOVE OBJECTS OR SCENE AUTOMATICALLY TO A CERTAIN LOCATION (ALL IMPLICITLY)
# ==========================================================================
def AutoMoveTo(x=None, y=None, z=None, steps=None, wait=None):
  command='AutoMoveTo '
  if (x!=None): command+='X='+cstr(x)+','
  if (y!=None): command+='Y='+cstr(y)+','
  if (z!=None): command+='Z='+cstr(z)+','
...
# MOVE OBJECTS OR SCENE AUTOMATICALLY TO A CERTAIN LOCATION (ALL)
# ===============================================================
def AutoMoveToAll(x=None, y=None, z=None, steps=None, wait=None):
  command='AutoMoveToAll '
  if (x!=None): command+='X='+cstr(x)+','
  if (y!=None): command+='Y='+cstr(y)+','
  if (z!=None): command+='Z='+cstr(z)+','
  if (steps!=None): command+='Steps='+cstr(steps)+','
...
# MOVE OBJECTS OR SCENE AUTOMATICALLY TO A CERTAIN LOCATION (OBJECT)
# ==================================================================
def AutoMoveToObj(selection1, x=None, y=None, z=None, steps=None, wait=None):
  command='AutoMoveToObj '
  command+=selstr(selection1)+','
  if (x!=None): command+='X='+cstr(x)+','
  if (y!=None): command+='Y='+cstr(y)+','
  if (z!=None): command+='Z='+cstr(z)+','
...
# MOVE OBJECTS OR SCENE AUTOMATICALLY (ALL IMPLICITLY)
# ====================================================
...
# ROTATE OBJECTS OR SCENE AUTOMATICALLY TO A CERTAIN ORIENTATION (ALL IMPLICITLY)
# ===============================================================================
def AutoRotateTo(alpha=None, beta=None, gamma=None, steps=None, wait=None):
  command='AutoRotateTo '
  if (alpha!=None): command+='Alpha='+cstr(alpha)+','
  if (beta!=None): command+='Beta='+cstr(beta)+','
  if (gamma!=None): command+='Gamma='+cstr(gamma)+','
  if (steps!=None): command+='Steps='+cstr(steps)+','
...
# ROTATE OBJECTS OR SCENE AUTOMATICALLY TO A CERTAIN ORIENTATION (ALL)
# ====================================================================
def AutoRotateToAll(alpha=None, beta=None, gamma=None, steps=None, wait=None):
  command='AutoRotateToAll '
  if (alpha!=None): command+='Alpha='+cstr(alpha)+','
  if (beta!=None): command+='Beta='+cstr(beta)+','
  if (gamma!=None): command+='Gamma='+cstr(gamma)+','
  if (steps!=None): command+='Steps='+cstr(steps)+','
...
# ROTATE OBJECTS OR SCENE AUTOMATICALLY TO A CERTAIN ORIENTATION (OBJECT)
# =======================================================================
def AutoRotateToObj(selection1, alpha=None, beta=None, gamma=None, steps=None, wait=None):
  command='AutoRotateToObj '
  command+=selstr(selection1)+','
  if (alpha!=None): command+='Alpha='+cstr(alpha)+','
  if (beta!=None): command+='Beta='+cstr(beta)+','
  if (gamma!=None): command+='Gamma='+cstr(gamma)+','
...
# ROTATE OBJECTS OR SCENE AUTOMATICALLY (ALL IMPLICITLY)
# ======================================================
...
# STYLE ATOMS AS BALLS&STICKS (ALL IMPLICITLY)
# ============================================
def BallStick():
  command='BallStick '
  return(runretval(command[:-1],retvalused()))
...
# STYLE ATOMS AS BALLS (ALL IMPLICITLY)
# =====================================
def Ball():
  command='Ball '
  return(runretval(command[:-1],retvalused()))
...
# SET/GET THE B-FACTOR (ALL IMPLICITLY)
# =====================================
...
# BLAST PROTEIN SEQUENCE (ALL IMPLICITLY)
# =======================================
...
# ====================
def BuildRes(name, psi=None, tau=None, center=None, isomer=None):
  command='BuildRes '
  command+='Name='+cstr(name)+','
  if (psi!=None): command+='Psi='+cstr(psi)+','
  if (tau!=None): command+='Tau='+cstr(tau)+','
...
# CALCULATE CAVITY VOLUMES (ALL IMPLICITLY)
# =========================================
...
# ==================================
# THIS IS ALTERNATIVE 2, WITH DIFFERENT PARAMETERS
def CellAuto(extension=None, selection1=None):
  command='Cell Auto,'
...
# CENTER ATOMS OR POLYGON MESHES (ALL IMPLICITLY)
# ===============================================
...
# SET/GET THE SUMMED UP CHARGE (ALL IMPLICITLY)
# =============================================
def Charge(e=None):
  command='Charge '
...
# CHECK STRUCTURE QUALITY (ALL IMPLICITLY)
# ========================================
...
# CLEAN OBJECTS FOR MOLECULAR DYNAMICS SIMULATION (ALL IMPLICITLY)
# ================================================================
def Clean(skip=None):
  command='Clean '
...
# ============================
def ColorPar(scheme, name, color, value=None):
  command='ColorPar '
...
# SET/GET ATOM COLORS (ALL IMPLICITLY)
# ====================================
def Color(first=None, second=None, segments=None, mapcons=None, filename=None):
  command='Color '
...
# ====================================
# THIS IS ALTERNATIVE 2, WITH DIFFERENT PARAMETERS
def Color2(noname1=None, filename=None):
  command='Color '
...
# =========================
def ColorAll(first=None, second=None, segments=None, mapcons=None, filename=None):
  command='ColorAll '
  if (first!=None): command+='first='+cstr(first)+','
  if (second!=None): command+='second='+cstr(second)+','
  if (segments!=None): command+='Segments='+cstr(segments)+','
...
# =========================
# THIS IS ALTERNATIVE 2, WITH DIFFERENT PARAMETERS
def ColorAll2(noname1=None, filename=None):
  command='ColorAll '
  if (noname1!=None): command+=cstr(noname1)+','
  if (filename!=None): command+='Filename='+cstr(filename)+','
  return(runretval(command[:-1],retvalused()))
...
# ============================
def ColorObj(selection1, first=None, second=None, segments=None, mapcons=None, filename=None):
  command='ColorObj '
  command+=selstr(selection1)+','
  if (first!=None): command+='first='+cstr(first)+','
  if (second!=None): command+='second='+cstr(second)+','
...
# ============================
# THIS IS ALTERNATIVE 2, WITH DIFFERENT PARAMETERS
def ColorObj2(selection1, noname2=None, filename=None):
  command='ColorObj '
  command+=selstr(selection1)+','
  if (noname2!=None): command+=cstr(noname2)+','
  if (filename!=None): command+='Filename='+cstr(filename)+','
  return(runretval(command[:-1],retvalused()))

# SET/GET ATOM COLORS (MOLECULE)
# ==============================
def ColorMol(selection1, first=None, second=None, segments=None, mapcons=None, filename=None):
  command='ColorMol '
  command+=selstr(selection1)+','
  if (first!=None): command+='first='+cstr(first)+','
  if (second!=None): command+='second='+cstr(second)+','
...
# ==============================
# THIS IS ALTERNATIVE 2, WITH DIFFERENT PARAMETERS
def ColorMol2(selection1, noname2=None, filename=None):
  command='ColorMol '
  command+=selstr(selection1)+','
  if (noname2!=None): command+=cstr(noname2)+','
  if (filename!=None): command+='Filename='+cstr(filename)+','
...
# =============================
def ColorRes(selection1, first=None, second=None, segments=None, mapcons=None, filename=None):
  command='ColorRes '
  command+=selstr(selection1)+','
  if (first!=None): command+='first='+cstr(first)+','
  if (second!=None): command+='second='+cstr(second)+','
...
# =============================
# THIS IS ALTERNATIVE 2, WITH DIFFERENT PARAMETERS
def ColorRes2(selection1, noname2=None, filename=None):
  command='ColorRes '
  command+=selstr(selection1)+','
  if (noname2!=None): command+=cstr(noname2)+','
  if (filename!=None): command+='Filename='+cstr(filename)+','
...
# ==========================
def ColorAtom(selection1, first=None, second=None, segments=None, mapcons=None, filename=None):
  command='ColorAtom '
  command+=selstr(selection1)+','
  if (first!=None): command+='first='+cstr(first)+','
  if (second!=None): command+='second='+cstr(second)+','
...
# ==========================
# THIS IS ALTERNATIVE 2, WITH DIFFERENT PARAMETERS
def ColorAtom2(selection1, noname2=None, filename=None):
  command='ColorAtom '
  command+=selstr(selection1)+','
  if (noname2!=None): command+=cstr(noname2)+','
  if (filename!=None): command+='Filename='+cstr(filename)+','
...
def CopyFile(filename, filename2):
  command='CopyFile '
  command+='Filename='+cstr(filename)+','
  command+='Filename='+cstr(filename2)+','
  return(runretval(command[:-1],retvalused()))
...
# CRYSTALLIZE OBJECTS TO FILL THE UNIT CELL (ALL IMPLICITLY)
# ==========================================================
...
# CUT OBJECTS OPEN (ALL IMPLICITLY)
# =================================
def Cut(secstr=None):
  command='Cut '
  if (secstr!=None): command+='SecStr='+cstr(secstr)+','
...
# CONVERT 3D TO 2D COORDINATES, CREATING A FLAT STRUCTURAL FORMULA (ALL IMPLICITLY)
# =================================================================================
...
# DELETE ALL HYDROGENS (ALL IMPLICITLY)
# =====================================
def DelHyd():
  command='DelHyd '
  return(runretval(command[:-1],retvalused()))
...
# DELETE RESTRAINTS (ALL IMPLICITLY)
# ==================================
...
# DELETE ALL WATER MOLECULES (ALL IMPLICITLY)
# ===========================================
def DelWater():
  command='DelWater '
  return(runretval(command[:-1],retvalused()))
...
# DUPLICATE OBJECTS (ALL IMPLICITLY)
# ==================================
def Duplicate():
  command='Duplicate '
  return(runretval(command[:-1],retvalused()))
...
# SET ENERGY UNIT
# ===============
def EnergyUnit(noname1):
  command='EnergyUnit '
  command+=cstr(noname1)+','
  return(runretval(command[:-1],retvalused()))

# CALCULATE FORCE FIELD ENERGIES (ALL IMPLICITLY)
# ===============================================
...
# ==============================
# THIS IS ALTERNATIVE 3, WITH DIFFERENT PARAMETERS
def ExperimentNeutralization(waterdensity=None, nacl=None, ph=None, pkafile=None, speed=None):
...
def ExperimentDocking(method=None, ligandobj=None, receptorobj=None, runs=None, clusterrmsd=None, resultfile=None, tmpfileid=None, gridparlist=None, dockparlist=None):
...
# ================================
def FillCellObj(selection1, copies=None, density=None, bumpsum=None, randomori=None):
  command='FillCellObj '
  command+=selstr(selection1)+','
  if (copies!=None): command+='Copies='+cstr(copies)+','
...
# FIX ATOMS DURING SIMULATION (ALL IMPLICITLY)
# ============================================
def Fix():
  command='Fix '
  return(runretval(command[:-1],retvalused()))
...
# SET/GET FORCE ON ATOMS (ALL IMPLICITLY)
# =======================================
...
# CALCULATE FORMATION ENERGIES (ALL IMPLICITLY)
# =============================================
def FormEnergy():
  command='FormEnergy '
  return(runretval(command[:-1],retvalused()))
...
# FREE ATOMS DURING SIMULATION (ALL IMPLICITLY)
# =============================================
def Free():
  command='Free '
  return(runretval(command[:-1],retvalused()))
...
# GRAB OBJECT OR SCENE FOR MOUSE MOVEMENT (ALL IMPLICITLY)
# ========================================================
def Grab():
  command='Grab '
  return(runretval(command[:-1],retvalused()))
...
# =======================================
def GroupAngle(selection1, selection2):
  command='GroupAngle '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  result=runretval(command[:-1],retvalused())
...
# ==========================================
def GroupDistance(selection1, selection2):
...
# ADD ATOMS TO GROUP (ALL IMPLICITLY)
# ===================================
def Group(name):
  command='Group '
  command+='Name='+cstr(name)+','
...
# HIDE HYDROGEN BONDS (ALL IMPLICITLY)
# ====================================
def HideHBo():
  command='HideHBo '
  return(runretval(command[:-1],retvalused()))
...
# HIDE RESTRAINTS (ALL IMPLICITLY)
# ================================
...
# HIDE SECONDARY STRUCTURE (ALL IMPLICITLY)
# =========================================
...
# GENERATE 3D COORDINATES FOR FLAT OR DISTORTED MOLECULES (ALL IMPLICITLY)
# ========================================================================
def Inflate():
  command='Inflate '
  return(runretval(command[:-1],retvalused()))
...
# CALCULATE KINETIC ENERGY (ALL IMPLICITLY)
# =========================================
...
# ==========================
def LightSource(alpha=None, gamma=None, ambience=None, shadow=None, shadowspeed=None):
  command='LightSource '
  if (alpha!=None): command+='Alpha='+cstr(alpha)+','
  if (gamma!=None): command+='Gamma='+cstr(gamma)+','
...
# FIND BONDS AUTOMATICALLY (ALL IMPLICITLY)
# =========================================
...
# ============================
def ListHBoObj(selection1, selection2, Min, results=None):
  command='ListHBoObj '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  command+='Min='+cstr(Min)+','
  if (results!=None): command+='Results='+cstr(results)+','
  return(runretval(command[:-1],retvalused()))

# LIST HYDROGEN BONDS (MOLECULE)
# ==============================
def ListHBoMol(selection1, selection2, Min, results=None):
  command='ListHBoMol '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  command+='Min='+cstr(Min)+','
...
# =============================
def ListHBoRes(selection1, selection2, Min, results=None):
  command='ListHBoRes '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  command+='Min='+cstr(Min)+','
...
# ==========================
def ListHBoAtom(selection1, selection2, Min, results=None):
  command='ListHBoAtom '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  command+='Min='+cstr(Min)+','
...
# LOAD SIMULATION SNAPSHOT IN MDCRD FORMAT (ALL IMPLICITLY)
# =========================================================
...
# LOAD SIMULATION SNAPSHOT IN XTC FORMAT (ALL IMPLICITLY)
# =======================================================
...
# LIST BONDS
# ==========
def ListBond(selection1, selection2, results=None):
  command='ListBond '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  if (results!=None): command+='Results='+cstr(results)+','
...
# LIST FLOATING ASSIGNMENTS (ALL IMPLICITLY)
# ==========================================
...
# LIST RESTRAINTS AND ENERGIES (ALL IMPLICITLY)
# =============================================
def ListRest(Class=None, component=None, format=None, sort=None, dismin=None, dihmin=None, objectsmin=None):
  command='ListRest '
  if (Class!=None): command+='Class='+cstr(Class)+','
  if (component!=None): command+='Component='+cstr(component)+','
  if (format!=None): command+='Format='+cstr(format)+','
...
# ==================================
def ListRestAll(Class=None, component=None, format=None, sort=None, dismin=None, dihmin=None, objectsmin=None):
  command='ListRestAll '
  if (Class!=None): command+='Class='+cstr(Class)+','
  if (component!=None): command+='Component='+cstr(component)+','
  if (format!=None): command+='Format='+cstr(format)+','
...
# =====================================
def ListRestObj(selection1, Class=None, component=None, format=None, sort=None, dismin=None, dihmin=None, objectsmin=None):
  command='ListRestObj '
  command+=selstr(selection1)+','
  if (Class!=None): command+='Class='+cstr(Class)+','
  if (component!=None): command+='Component='+cstr(component)+','
...
# =======================================
def ListRestMol(selection1, Class=None, component=None, format=None, sort=None, dismin=None, dihmin=None, objectsmin=None):
  command='ListRestMol '
  command+=selstr(selection1)+','
  if (Class!=None): command+='Class='+cstr(Class)+','
  if (component!=None): command+='Component='+cstr(component)+','
...
# ======================================
def ListRestRes(selection1, Class=None, component=None, format=None, sort=None, dismin=None, dihmin=None, objectsmin=None):
  command='ListRestRes '
  command+=selstr(selection1)+','
  if (Class!=None): command+='Class='+cstr(Class)+','
  if (component!=None): command+='Component='+cstr(component)+','
...
# ===================================
def ListRestAtom(selection1, Class=None, component=None, format=None, sort=None, dismin=None, dihmin=None, objectsmin=None):
...
# MOVE ATOMS, OBJECTS OR THE SCENE (ALL IMPLICITLY)
# =================================================
...
# ORIENT OBJECTS NICELY (ALL IMPLICITLY)
# ======================================
...
# SET/GET THE OCCUPANCY (ALL IMPLICITLY)
# ======================================
...
# OLIGOMERIZE OBJECTS TO GENERATE THE BIOLOGICALLY ACTIVE FORM (ALL IMPLICITLY)
# =============================================================================
def Oligomerize(center=None):
  command='Oligomerize '
  if (center!=None): command+='Center='+cstr(center)+','
  return(runretval(command[:-1],retvalused()))
...
# ==================================================================
def OligomerizeAll(center=None):
...
# =====================================================================
def OligomerizeObj(selection1, center=None):
...
# OPTIMIZE HYDROGEN BONDING NETWORK (ALL IMPLICITLY)
# ==================================================
def OptHyd(method):
  command='OptHyd '
...
# OPTIMIZE MOLECULAR GEOMETRY (ALL IMPLICITLY)
# ============================================
...
# SET/GET OBJECT OR SCENE ORIENTATION (ALL IMPLICITLY)
# ====================================================
...
# SET/GET OBJECT OR SCENE POSITION AND ORIENTATION (ALL IMPLICITLY)
# =================================================================
...
# SET/GET OBJECT OR SCENE POSITION (ALL IMPLICITLY)
# =================================================
...
# SET/GET THE PROPERTY VALUE (ALL IMPLICITLY)
# ===========================================
def Prop(value=None):
  command='Prop '
  if (value!=None): command+='value='+cstr(value)+','
...
# CALCULATE THE RADIUS (ALL IMPLICITLY)
# =====================================
...
# REGULARIZE COVALENT GEOMETRY (ALL IMPLICITLY)
# =============================================
...
# REMOVE FROM ENVIRONMENT FOR SURFACE CALCULATIONS (ALL IMPLICITLY)
# =================================================================
def RemoveEnv():
  command='RemoveEnv '
  return(runretval(command[:-1],retvalused()))
...
# REMOVE OBJECTS FROM THE SOUP (ALL IMPLICITLY)
# =============================================
def Remove():
  command='Remove '
  return(runretval(command[:-1],retvalused()))
...
# PREDICT SECONDARY STRUCTURE (ALL IMPLICITLY)
# ============================================
...
# RENUMBER OBJECTS (ALL IMPLICITLY)
# =================================
def Renumber(first=None):
  command='Renumber '
  if (first!=None): command+='first='+cstr(first)+','
  return(runretval(command[:-1],retvalused()))

# RENUMBER OBJECTS (ALL)
# ======================
def RenumberAll(first=None):
  command='RenumberAll '
  if (first!=None): command+='first='+cstr(first)+','
  return(runretval(command[:-1],retvalused()))

# RENUMBER OBJECTS (OBJECT)
# =========================
def RenumberObj(selection1, first=None):
  command='RenumberObj '
  command+=selstr(selection1)+','
  if (first!=None): command+='first='+cstr(first)+','
  return(runretval(command[:-1],retvalused()))
...
# RENUMBER RESIDUES
# =================
def RenumberRes(selection1, first=None, inscode=None, increment=None):
  command='RenumberRes '
  command+=selstr(selection1)+','
  if (first!=None): command+='First='+cstr(first)+','
...
# CALCULATE RESTRAINT ENERGIES (ALL IMPLICITLY)
# =============================================
...
# GET RESTRAINT VIOLATION STATISTICS (ALL IMPLICITLY)
# ===================================================
def RestViol():
  command='RestViol '
  return(runretval(command[:-1],retvalused()))
...
# ==============================
def RestrainPar(average=None, ceil=None, dismin=None, monomers=None, joindis=None, floatgroups=None):
  command='RestrainPar '
...
# ======================================
def RestrainPot(name=None, sqconstant=None, sqoffset=None, sqexponent=None, rswitch=None, soexponent=None, asymptote=None, gamma=None, update=None):
  command='RestrainPot '
  if (name!=None): command+='Name='+cstr(name)+','
...
# ROTATE ATOMS, OBJECTS OR THE SCENE (ALL IMPLICITLY)
# ===================================================
...
# ROTATE ATOMS ABOUT A SPECIFIED AXIS (ALL IMPLICITLY)
# ====================================================
...
# ======================
def SampleDih(selection1, method=None, structures=None, dihedrals=None, bumpsum=None, scaffold=None):
  command='SampleDih '
  command+=selstr(selection1)+','
  if (method!=None): command+='Method='+cstr(method)+','
  if (structures!=None): command+='Structures='+cstr(structures)+','
...
# ================================
def SaveESP(noname1, method=None):
  command='SaveESP '
  command+=cstr(noname1)+','
  if (method!=None): command+='Method='+cstr(method)+','
  # ALWAYS GET A RETURN VALUE TO SYNCHRONIZE THREADS
...
# ===========================
def SavePDB(selection1, filename, format=None, transform=None):
  command='SavePDB '
  command+=selstr(selection1)+','
  command+='Filename='+cstr(filename)+','
  if (format!=None): command+='Format='+cstr(format)+','
...
# SAVE AMBER PREP TOPOLOGY (ALL IMPLICITLY)
# =========================================
...
# =======================================
def SaveSim(filename, steps, number=None):
  command='SaveSim '
  command+='Filename='+cstr(filename)+','
  command+='Steps='+cstr(steps)+','
  if (number!=None): command+='Number='+cstr(number)+','
...
# SCALE RESTRAINTS (ALL IMPLICITLY)
# =================================
def ScaleRest(Class, distance, dihedral):
  command='ScaleRest '
  command+='Class='+cstr(Class)+','
  command+='Distance='+cstr(distance)+','
  command+='Dihedral='+cstr(dihedral)+','
  return(runretval(command[:-1],retvalused()))

# SCALE RESTRAINTS (ALL)
# ======================
def ScaleRestAll(Class, distance, dihedral):
...
# =========================
def ScaleRestObj(selection1, Class, distance, dihedral):
...
# ===========================
def ScaleRestMol(selection1, Class, distance, dihedral):
...
# ==========================
def ScaleRestRes(selection1, Class, distance, dihedral):
...
# =======================
def ScaleRestAtom(selection1, Class, distance, dihedral):
...
# SCALE ATOM POSITIONS AND POLYGON MESHES (ALL IMPLICITLY)
# ========================================================
...
# SET/GET SECONDARY STRUCTURE (ALL IMPLICITLY)
# ============================================
...
# SET/GET THE SEGMENT NAME (ALL IMPLICITLY)
# =========================================
def Seg(name=None):
  command='Seg '
  if (name!=None): command+='Name='+cstr(name)+','
...
# GET RESIDUE SEQUENCE (ALL IMPLICITLY)
# =====================================
...
# SHOW HYDROGEN BONDS (ALL IMPLICITLY)
# ====================================
...
# ==========================
def ShowIntObj(selection1, selection2, *arglist2):
  command='ShowIntObj '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  # ANY NUMBER OF VARIABLE ARGUMENTS CAN FOLLOW
...
# ============================
def ShowIntMol(selection1, selection2, *arglist2):
  command='ShowIntMol '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  # ANY NUMBER OF VARIABLE ARGUMENTS CAN FOLLOW
...
# ===========================
def ShowIntRes(selection1, selection2, *arglist2):
  command='ShowIntRes '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  # ANY NUMBER OF VARIABLE ARGUMENTS CAN FOLLOW
...
# ========================
def ShowIntAtom(selection1, selection2, *arglist2):
  command='ShowIntAtom '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  # ANY NUMBER OF VARIABLE ARGUMENTS CAN FOLLOW
...
# SHOW ION BINDING SITES (ALL IMPLICITLY)
# =======================================
...
# SHOW RESTRAINTS (ALL IMPLICITLY)
# ================================
...
# SHOW SIDE-CHAIN ROTAMERS (ALL IMPLICITLY)
# =========================================
def ShowRota():
  command='ShowRota '
  return(runretval(command[:-1],retvalused()))
...
# SHOW SECONDARY STRUCTURE (ALL IMPLICITLY)
# =========================================
...
# ====================
def ShowSimGrid(Type=None, center=None, color=None):
  command='ShowSimGrid '
  if (Type!=None): command+='Type='+cstr(Type)+','
  if (center!=None): command+='Center='+cstr(center)+','
  if (color!=None): command+='Color='+cstr(color)+','
...
# ==========================
def ShowSphere(radius, color, alpha=None, level=None, scaley=None, scalez=None):
  command='ShowSphere '
  command+='Radius='+cstr(radius)+','
  command+='Color='+cstr(color)+','
  if (alpha!=None): command+='Alpha='+cstr(alpha)+','
...
# SHOW POLYGON INSIDE RING (ALL IMPLICITLY)
# =========================================
...
# ======================================================
def ShowWin(selection1, title, *arglist2):
  command='ShowWin '
  command+=selstr(selection1)+','
...
# SHOW POLYGON MESH AS WIRE FRAME (ALL IMPLICITLY)
# ================================================
...
# CALCULATE SOLVATION ENERGY (ALL IMPLICITLY)
# ===========================================
...
# SET/GET SPEED AND VELOCITY OF ATOMS (ALL IMPLICITLY)
# ====================================================
...
# SPLIT OBJECTS AT SPLIT POINTS (ALL IMPLICITLY)
# ==============================================
def Split(center=None):
  command='Split '
  if (center!=None): command+='Center='+cstr(center)+','
  return(runretval(command[:-1],retvalused()))
...
# ======================================
def SplitObj(selection1, center=None):
  command='SplitObj '
  command+=selstr(selection1)+','
  if (center!=None): command+='Center='+cstr(center)+','
  return(runretval(command[:-1],retvalused()))
...
# STYLE ATOMS AS STICKS (ALL IMPLICITLY)
# ======================================
def Stick():
  command='Stick '
  return(runretval(command[:-1],retvalused()))
...
# =========================
def Style(backbone=None, sidechain=None, save=None):
  command='Style '
...
# ==========================
def SurfPar(probe=None, resolution=None, molecular=None, espmax=None, smoothcut=None, radii=None):
  command='SurfPar '
  if (probe!=None): command+='Probe='+cstr(probe)+','
  if (resolution!=None): command+='Resolution='+cstr(resolution)+','
...
# CALCULATE ELECTROSTATIC SURFACE POTENTIALS (ALL IMPLICITLY)
# ===========================================================
...
# CALCULATE SURFACE AREAS (ALL IMPLICITLY)
# ========================================
...
# ==============================
def SwapAtom(selection1, element, updatebonds=None, updatehyd=None, rename=None):
  command='SwapAtom '
  command+=selstr(selection1)+','
  command+='Element='+cstr(element)+','
...
# SWAP HYDROGEN ORDERING IN RESIDUES (ALL IMPLICITLY)
# ===================================================
def SwapHyd(order):
  command='SwapHyd '
...
# SWAP ATOM POSITIONS
# ===================
def SwapPosAtom(selection1, selection2):
  command='SwapPosAtom '
  command+=selstr(selection1)+','
  command+=selstr(selection2)+','
  return(runretval(command[:-1],retvalused()))
...
# SWITCH OBJECTS ON/OFF (ALL IMPLICITLY)
# ======================================
def Switch(visibility, wait=None):
  command='Switch '
  command+='Visibility='+cstr(visibility)+','
  if (wait!=None): command+='Wait='+cstr(wait)+','
  return(runretval(command[:-1],retvalused()))

# SWITCH OBJECTS ON/OFF (ALL)
# ===========================
def SwitchAll(visibility, wait=None):
  command='SwitchAll '
  command+='Visibility='+cstr(visibility)+','
  if (wait!=None): command+='Wait='+cstr(wait)+','
  return(runretval(command[:-1],retvalused()))
...
# ==============================
def SwitchObj(selection1, visibility, wait=None):
  command='SwitchObj '
  command+=selstr(selection1)+','
  command+='Visibility='+cstr(visibility)+','
  if (wait!=None): command+='Wait='+cstr(wait)+','
...
# TRANSFER OBJECTS INTO ANOTHER COORDINATE SYSTEM (ALL IMPLICITLY)
# ================================================================
...
# TRANSFORM OBJECTS (ALL IMPLICITLY)
# ==================================
...
# GET THE ATOM TYPE
# =================
def TypeAtom(selection1, method):
  command='TypeAtom '
  command+=selstr(selection1)+','
  command+='Method='+cstr(method)+','
  return(runretval(command[:-1],retvalused()))
...
# REMOVE ATOMS FROM GROUP (ALL IMPLICITLY)
# ========================================
def Ungroup(name):
  command='Ungroup '
  command+='Name='+cstr(name)+','
...
# CALCULATE VOLUMES (ALL IMPLICITLY)
# ==================================
...
# ZOOM IN ON ATOMS (ALL IMPLICITLY)
# =================================
